...
function FeaturedHotspotsComponent({
  hotspots,
  wishlistIds = [],
  onWishlistToggle,
  selectedCategory = "",
  ...
  const filteredHotspots = useMemo(() => {
    return selectedCategory ? hotspots.filter((h) => h.category === selectedCategory) : hotspots;
  ...
  }, [checkOverflow, filteredHotspots.length]);
    ...
    <section className="py-8 bg-white rounded-lg">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl md:text-2xl font-bold text-slate-900">Hidden gems worth exploring</h2>
            <p className="text-sm text-slate-600 mt-1">Handpicked adventures across Belgium.</p>
          </div>
        ...
        >
          {filteredHotspots.map((hotspot, index) => {
            ...
            const isWishlisted = wishlistSet.has(hotspot.id);
                ...
                <Link href={`/hotspots/${hotspot.id}`}>
                  <div className="group relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                    <div className="relative h-40 md:h-44 overflow-hidden">
                      {/* Keep src/props stable so the image does not remount/reload on unrelated parent renders. */}
                      <OptimizedImage
                        src={imageUrl}
                        alt={hotspot.name}
                        fill
                      ...
                      <div
                        className={`absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-medium text-white ${getCategoryColor(
                          hotspot.category
                        )}`}
                      >
                      ...
                      {onWishlistToggle && (
                        ...
                        >
                          <Heart className={`w-4 h-4 ${isWishlisted ? "fill-amber-500 text-amber-500" : "text-slate-600"}`} />
                        </button>
                      )}
                    </div>
                    ...
                    <div className="p-4">
                      ...
                      {hotspot.visit_count !== undefined && hotspot.visit_count > 0 && (
                        <p className="text-xs text-slate-500 mt-2">{hotspot.visit_count} visits</p>
                      )}
                    </div>
                  </div>
                </Link>
        ...
        <div className="md:hidden mt-4 text-center">
          <Link href="/hotspots" className="inline-flex items-center gap-1 text-emerald-600 font-medium text-sm">
            View all hotspots
            <ChevronRight className="w-4 h-4" />
          </Link>
        </div>
...
const FeaturedHotspots = memo(FeaturedHotspotsComponent);
FeaturedHotspots.displayName = "FeaturedHotspots";
